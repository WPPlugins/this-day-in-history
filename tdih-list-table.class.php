...
class TDIH_List_Table extends WP_List_Table {
	...
	private function date_reorder($date) {
		...
		switch ($this->date_format) {
			...
			case '%m-%d-%Y':
				if (preg_match("/^(\d{2})-(\d{2})-(\d{1,4})$/", $date, $matches)) {
					return sprintf('%04d', $matches[3]).'-'.$matches[1].'-'.$matches[2];
				}
				break;

			case '%d-%m-%Y':
				if (preg_match("/^(\d{2})-(\d{2})-(\d{1,4})$/", $date, $matches)) {
					return sprintf('%04d', $matches[3]).'-'.$matches[2].'-'.$matches[1];
				}
				break;

			case '%Y-%m-%d':
				if (preg_match("/^(\d{1,4})-(\d{2})-(\d{2})$/", $date, $matches)) {
					return sprintf('%04d', $matches[1]).'-'.$matches[2].'-'.$matches[3];
				}
			break;
		}
